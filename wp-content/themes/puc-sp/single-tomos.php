...
/**
 * Template para exibir um Tomo individual
 * Lista verbetes relacionados (campo ACF relationship "verbetes")
...
$tomo_id = get_the_ID();
$versao_ativa = puc_sp_get_tomo_versao_ativa($tomo_id);
$codigo = $versao_ativa['codigo'];
$coordenadores = $versao_ativa['coordenadores'];
...
// Busca todos os verbetes que têm este tomo vinculado no campo 'edicoes'
$verbetes_data = [];
...
$query_args = [
	...
	'meta_query'     => [
		[
			'key'     => 'edicoes', // Campo ACF relationship nos verbetes que vincula aos tomos
			'value'   => '"' . $tomo_id . '"', // WordPress serializa relationships com aspas
			'compare' => 'LIKE',
		],
	],
];
...
if ($related_query->have_posts()) {
	while ($related_query->have_posts()) {
		...
		$verbetes_data[] = [
			...
			'letter'  => puc_sp_normalize_first_letter($title),
		];
	}
	wp_reset_postdata();
}
			...
			<div class="">
				<nav class="mb-4">
					<ol class="flex items-center gap-2 text-white/70 text-sm">
						...
						<li><a href="<?php echo home_url('/tomos'); ?>" class="hover:text-white transition-colors">Tomos</a></li>
						<li><span class="mx-2">/</span></li>
				...
				<?php if ($codigo) : ?>
					<span class="inline-block text-xs font-semibold text-app-blue bg-white px-2 py-1 rounded mt-4">
						<?php echo esc_html($codigo); ?>
					</span>
				<?php endif; ?>

				<?php if ($coordenadores) : ?>
					<p class="text-white/80 mt-2 text-sm"><strong>Coordenação:</strong> <?php echo esc_html($coordenadores); ?></p>
				<?php endif; ?>
				...
				<form action="<?php echo get_permalink(); ?>" method="get" class="flex items-center gap-3 mt-6">
					<div class="relative w-full max-w-80">
						<input
							...
							placeholder="Buscar verbetes deste tomo..."
...
<!-- Listagem de verbetes relacionados -->
